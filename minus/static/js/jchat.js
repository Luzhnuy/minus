...
function sync_messages() {
    $.ajax({
		...
		success: function (json) {
            ...
            //get not the latest one, but 10
            last_received = last_received - 10; 
		}        
    });
		...
		success: function (json) {
			...
			$.each(json, function(i,m){
				if (m.type == 's')
					...
					$('#chat-messages').append('<div class="join">Увійшов '+m.author+'</div>');
				else if (m.type == 'l') 	
					$('#chat-messages').append('<div class="leave">'+m.author+' покинув чат</div>');
